...
nRF24::nRF24(SPIClass* _spi, uint8_t _csn, uint8_t _ce) :
	_SPI(_spi), pinCSN(_csn), pinCE(_ce), p_variant(false),
...
bool nRF24::begin() {
	...
	if (pinCE != pinCSN) pinMode(pinCE,  OUTPUT);
	if (pinCE != pinCSN) pinMode(pinCSN, OUTPUT);
    ...
    //
	printlnDebug("setDataRate RF24_250KBPS");
    ...
    // hardware.
	printlnDebug("setDataRate RF24_1MBPS");
    setDataRate( RF24_1MBPS ) ;
    
    // Initialize CRC and request 2-byte (16bit) CRC
	printlnDebug("setCRCLength RF24_CRC_16");
    setCRCLength( RF24_CRC_16 ) ;
    
    // Disable dynamic payloads, to match dynamic_payloads_enabled setting - Reset value is 0
	printlnDebug("Disable dynamic payloads");
    //toggle_features();
	...
	printlnDebug("Reset current status");
...
/****************************************************************************/
void nRF24::csn(bool mode)
{
	digitalWrite(pinCSN, mode);
	delayMicroseconds(csDelay);
}

void nRF24::ce(bool level)
{
  if (pinCE != pinCSN) digitalWrite(pinCE, level);
}

/****************************************************************************/
void nRF24::beginTransaction() {
	csn(LOW);
}

void nRF24::endTransaction() {
	csn(HIGH);
}

/****************************************************************************/
void nRF24::startListening(void) {
  printlnDebug("startListening");
  powerUp();
  write_register(NRF_CONFIG, read_register(NRF_CONFIG) | _BV(PRIM_RX));
  write_register(NRF_STATUS, _BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
  ce(HIGH);
...
/****************************************************************************/
uint8_t nRF24::write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
  uint8_t status;
...
/****************************************************************************/
uint8_t nRF24::read_payload(void* buf, uint8_t data_len)
{
  uint8_t status;
...
/****************************************************************************/
uint8_t nRF24::spiTrans(uint8_t cmd) {
  uint8_t status;
...
void nRF24::setChannel(uint8_t channel) {
  printfDebug("setChannel: %d\r\n", channel);
  const uint8_t max_channel = 125;
  write_register(RF_CH, min(channel, max_channel));
}

uint8_t nRF24::getChannel() {
  uint8_t channel = read_register(RF_CH);
  printfDebug("getChannel: %d\r\n", channel);
  return channel;
}
...
void nRF24::setPayloadSize(uint8_t size) {
  ...
  printfDebug("setPayloadSize: %d\r\n", payload_size);
}

uint8_t nRF24::getPayloadSize(void) {
  printfDebug("getPayloadSize: %d\r\n", payload_size);
  return payload_size;
}

/****************************************************************************/
void nRF24::setAddressWidth(uint8_t a_width) {
	if(a_width -= 2) {
    ...
    printfDebug("setAddressWidth: %d\r\n", addr_width);
}

/****************************************************************************/
bool nRF24::rxFifoFull() {
	return read_register(FIFO_STATUS) & _BV(RX_FULL);
}

/****************************************************************************/
bool nRF24::txStandBy() {
    printlnDebug("txStandBy");
...
bool nRF24::txStandBy(uint32_t timeout, bool startTx) {
    printfDebug("txStandBy: %d [%d]\r\n", timeout, startTx);
    if(startTx) {
	  stopListening();
	  ce(HIGH);
	}
...
/****************************************************************************/
void nRF24::maskIRQ(bool tx, bool fail, bool rx) {
...
uint8_t nRF24::getDynamicPayloadSize(void) {
  ...
  printfDebug("getDynamicPayloadSize: %d\r\n", result);
  return result;
}
...
/****************************************************************************/
void nRF24::errNotify() {
	printlnDebug("HARDWARE FAIL: Radio not responding, verify pin connections, wiring, etc.");
	failureDetected = 1;
}
...
bool nRF24::write( const void* buf, uint8_t len, const bool multicast ) {
	//Start Writing
	startFastWrite(buf,len, multicast);
    //startWrite(buf, len, 0);
...
bool nRF24::write( const void* buf, uint8_t len ) {
	return write(buf,len, 0);
}

/****************************************************************************/
//For general use, the interrupt flags are not important to clear
bool nRF24::writeBlocking( const void* buf, uint8_t len, uint32_t timeout ) {
...
void nRF24::startFastWrite( const void* buf, uint8_t len, const bool multicast, bool startTx) { //TMRh20
...
void nRF24::startWrite( const void* buf, uint8_t len, const bool multicast ){
  // Send the payload
  //write_payload( buf, len );
  write_payload( buf, len, multicast ? W_TX_PAYLOAD_NO_ACK : W_TX_PAYLOAD ) ;
  ce(HIGH);
...
/****************************************************************************/
void nRF24::read( void* buf, uint8_t len ) {
  // Fetch the payload
...
/****************************************************************************/
void nRF24::openWritingPipe(uint64_t value) {
  printfDebug("openWritingPipe: 0x%08X\r\n", value);
...
void nRF24::openWritingPipe(const uint8_t *address) {
  printfDebug("openWritingPipe: 0x%08X\r\n", *address);
  // Note that AVR 8-bit uC's store this LSB first, and the NRF24L01(+)
  // expects it LSB first too, so we're good.
...
/****************************************************************************/
void nRF24::openReadingPipe(uint8_t child, uint64_t address) {
  printfDebug("openReadingPipe: 0x%08X for child %d\r\n", address, child);
...
void nRF24::openReadingPipe(uint8_t child, const uint8_t *address) {
  printfDebug("openReadingPipe: 0x%08X for child %d\r\n", *address, child);
  // If this is pipe 0, cache the address.  This is needed because
  // openWritingPipe() will overwrite the pipe 0 address, so
  // startListening() will have to restore it.
...
void nRF24::closeReadingPipe( uint8_t pipe ) {
  printfDebug("closeReadingPipe: %d\r\n", pipe);
...
void nRF24::enableDynamicPayloads(void) {
    printlnDebug("enableDynamicPayloads");
...
void nRF24::disableDynamicPayloads(void) {
    printlnDebug("disableDynamicPayloads");
...
/****************************************************************************/
void nRF24::enableDynamicAck(void) {
    printlnDebug("enableDynamicAck");
...
/****************************************************************************/
bool nRF24::isPVariant(void) {
  return p_variant;
}

/****************************************************************************/
void nRF24::setAutoAck(bool enable) {
    printfDebug("setAutoAck: %d\r\n", enable);
    if ( enable ) {
...
void nRF24::setAutoAck( uint8_t pipe, bool enable ) {
  printfDebug("setAutoAck: %d for pipe %d\r\n", enable, pipe);
  if ( pipe <= 6 ) {
...
void nRF24::setPALevel(uint8_t level) {
  ...
  printfDebug("setPALevel: %d \r\n", level);
  write_register( RF_SETUP, setup |= level ) ;	// Write it to the chip
}

uint8_t nRF24::getPALevel(void) {
  uint8_t level = (read_register(RF_SETUP) & (_BV(RF_PWR_LOW) | _BV(RF_PWR_HIGH))) >> 1 ;
  //printfDebug("getPALevel: %d \r\n", level);
  return level;
}
...
void nRF24::printDetails(void) {
    ...
    Serial.println(pgm_read_word(&rf24_datarate_e_str_P[getDataRate()]));
    Serial.print("Model\t\t  = ");
    ...
    Serial.println(pgm_read_word(&rf24_pa_dbm_e_str_P[getPALevel()]));
  #else
...
/****************************************************************************/
void nRF24::print_status(uint8_t status) {
  #if defined (__STM32F1__)
    Serial.print("STATUS\t\t  = 0x");
    if( status < 0x10) Serial.print("0");
    Serial.print(status, HEX);
    Serial.print(" RX_DR=");
    Serial.print((status & _BV(RX_DR))?1:0,HEX);
    Serial.print(" TX_DS=");
    Serial.print((status & _BV(TX_DS))?1:0,HEX);
    Serial.print(" MAX_RT=");
    Serial.print((status & _BV(MAX_RT))?1:0,HEX);
    Serial.print(" RX_P_NO=");
    Serial.print(((status >> RX_P_NO) & B111),HEX);
    Serial.print(" TX_FULL=");
    Serial.print((status & _BV(TX_FULL))?1:0,HEX);
    Serial.print("\r\n");
  #else
...
void nRF24::print_observe_tx(uint8_t value) {
...
/****************************************************************************/
void nRF24::print_byte_register(const char* name, uint8_t reg, uint8_t qty) {
...
/****************************************************************************/
void nRF24::print_address_register(const char* name, uint8_t reg, uint8_t qty) {
  char extra_tab = strlen_P(name) < 8 ? '\t' : 0;
  #if defined (__STM32F1__)
    Serial.print(name);
    Serial.print("\t");
